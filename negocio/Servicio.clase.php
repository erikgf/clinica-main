...
class Servicio extends Conexion {
        ...
        try {
            ...
            if ($filtro != NULL){
                switch($filtro){
                    ...
                    break;
                    case "4":
                    $sqlFiltroCategoria = " arreglo_paquete IS NOT NULL AND se.id_categoria_servicio = '".$this->ID_CATEGORIA_LABORATORIO."' ";
                    break;
                }
            }
    ...
    public function obtener(){
        try {
            ...
            if ($servicio["arreglo_paquete"] != ""){
                $sql = "SELECT  
                            se.id_servicio, 
                            se.descripcion as nombre_servicio
                            FROM servicio se 
                            WHERE se.estado_mrcb AND se.id_servicio IN ({$servicio["arreglo_paquete"]})
                            ORDER BY se.descripcion";

                $servicio["servicios_paquete"] = $this->consultarFilas($sql);
            } else {
                $servicio["servicios_paquete"] = [];
            }
    ...
    public function registrarPaquete(){
        try {
            ...
            $arreglo_paquete = implode(',', $this->arreglo_detalle);
            ...
            $campos_valores = [
                ...
                "cantidad_examenes"=>"1",
                "idtipo_afectacion"=>$this->idtipo_afectacion,
                "arreglo_paquete"=>$arreglo_paquete
            ];
            ...
            $sql = "SELECT  id_servicio as id, 
                    ...
                    precio_venta_sin_igv as valor_venta,
                    COALESCE(comision * 100, '0.00') as porcentaje_comision,
                    'PAQUETE' as tipo_servicio,
                    '4' as id_tipo_servicio
                    FROM servicio se 
                    INNER JOIN categoria_servicio cat ON se.id_categoria_servicio =  cat.id_categoria_servicio
                    WHERE se.estado_mrcb  AND se.id_servicio = :0";
            $registro = $this->consultarFila($sql, [$this->id_servicio]);
    ...
    public function leerServicioPaquete(){
        try {
            $sql = "SELECT  
                    se.id_servicio, 
                    se.descripcion, 
                    ...
                    arreglo_paquete
                    FROM servicio se 
                    WHERE se.estado_mrcb AND se.id_servicio = :0 AND arreglo_paquete IS NOT NULL";
            $registro =  $this->consultarFila($sql, [$this->id_servicio]);

            if ($registro != false){
                $cadena_id_servicios = $registro["arreglo_paquete"];
                $sql = "SELECT 
                            id_servicio,
                            descripcion as nombre_servicio,
                            precio_unitario as precio_venta,
