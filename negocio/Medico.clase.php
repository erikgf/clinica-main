...
class Medico extends Conexion {
    ...
    public function buscar($cadenaBuscar){
        try {
            $sql = "SELECT 
                    id_medico as id,
                    TRIM(CONCAT(COALESCE(nombres,''), ' ', COALESCE(apellidos_paterno,''), ' ', COALESCE(apellidos_materno,''))) as text
                    FROM medico
                    WHERE estado_mrcb AND
                        CONCAT(COALESCE(nombres,''), ' ', COALESCE(apellidos_paterno,''),' ', COALESCE(apellidos_materno,'')) LIKE '%".$cadenaBuscar."%' 
                    LIMIT 5";
